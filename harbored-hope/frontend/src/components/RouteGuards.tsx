...
export function RequireAuth({ children, role }: { children: ReactNode; role?: 'Admin' | 'Donor' | 'Staff' }) {
  ...
  if (role === 'Donor' && !isDonor && !isAdmin) {
    // Staff-only accounts belong in the admin area
    if (isStaff) return <Navigate to="/admin" replace />;
    // New accounts may have no roles yet due to assignment timing — allow through
  }
...
export function RedirectIfAuthed({ children }: { children: ReactNode }) {
  ...
  if (user) {
    ...
    // Logged in but no recognized role yet — default to donor dashboard
